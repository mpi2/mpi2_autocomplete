    ...
    $.widget('MPI2.mpi2AutoComplete', $.ui.autocomplete, {	
		...
		_inputValMappingForCallBack: function(input, srchBtn){
			...
			//if ( input.indexOf(':') == -1 ){
				...
				// do nothing for now
				//console.log('MOUSE0: ' + solrQStr);
				//self._trigger("loadSideBar", null, { queryString: solrQStr });*/
			//}
			if ( srchBtn ){
				self.options.searchMode = self._setSearchMode(
				...
				if ( input.indexOf(':') != -1 ){					
					if ( typeof displayField != 'undefined' ){
						var srcLabelInv = $.fn.inverseSimpleJSON(self.options.srcLabel);						
						//console.log(srcLabelInv[displayField] + ':' + '"' + termVal + '"');
						
						// TO DO: sanger index is not working for search not using mgi_accession_id						
						solrQStr = srcLabelInv[displayField] + ':' + '"' + termVal + '"';
					}							
				}
			}
				...
				//console.log('MOUSE1: '+ solrQStr + ' -- ' + ' termVal: ' + termVal);	
			}	
			...
			//alert("before Enter Event1: "+ self.options.hitEnterBeforeDropDownListOpensVal);
			...
			self._trigger("loadGenePage", null, { q: solrQStr, type: self.options.searchMode, 
        ...
        _create : function () {        	    	
            ...
            // remove all facet counts when input box is empty/changed
            ...
            /*
            $('img#maFacetInfo').toggle(
            	function(){	$('span#maMsg').show(); },
            	function(){	$('span#maMsg').hide(); }
            );
            $('img#mpFacetInfo').toggle(
               	function(){	$('span#mpMsg').show(); },
               	function(){	$('span#mpMsg').hide(); }
            );*/
		...
		_parseJson: function(json, sQuery, sDataType, sDivId, aFields){			
			...
			/*if ( sDataType == 'ma' && matchesFound != 0 ) {
				$('img#maFacetInfo').show();
			}
			else if ( sDataType == 'mp' && matchesFound != 0 ){
				$('img#mpFacetInfo').show();
			}*/
		...
		_setSearchMode: function(oCounts){
			...
			else if ( oCounts.geneFound == 0 && oCounts.mpFound != 0){
				return 'phenotype';			
			}
    		else if ( oCounts.geneFound == 0 && oCounts.mpFound == 0 && oCounts.sopFound != 0 ){
    			return 'parameter';						
			}
    		else {
    			return 'gene'; //default
    	...
    	_doImageAutosuggest: function(geneSolrResponse, sopSolrResponse, maSolrResponse, q, response){
    		...
    		var queryParams = {    			   			    			
    			...
    			'fl' : 'annotationTermId,annotationTermName,expName',
    			'q': q    			
    		};
		...
		_doMPAutoSuggest: function(geneSolrResponse, sopSolrResponse, maSolrResponse, imgSolrResponse, q, response){
    		...
    		$.ajax({
        	    ...
        	    success: function (mpSolrResponse) {
        	    	...
        	    	//console.log('geneFound: ' + self.options.geneFound + ' - mpFound: ' + self.options.mpFound + ' - sopFound: ' + self.options.sopFound);
        	    	...
        	    	if ( response ){
        	    		...
        	    		response(self.options.acList.slice(0,3)); // return only first 4 terms in the list for now
        	    	} 
        	    	self._doCallBacks();        	    		
    	...
    	_doCallBacks: function(){
    		...
    		var queryStr = self.options.onSearch ? self.term : '*:*';    		
    		...
    		if ( window.location.pathname != self.options.search_pathname && self.options.hitEnterBeforeDropDownListOpensVal == 1 ){											
    			self._trigger("redirectedSearch", null, { q: queryStr, 
    		...
    		//console.log('search mode: '+ self.options.facetTypeParams[self.options.searchMode]);
    		params.q = self.term;
    		params.explaination = self.term;
    		params.geneFound = self.options.geneFound;
    		    		
    		//self._trigger("loadGenePage", null, { queryString: self.term, type: self.options.searchMode, geneFound: self.options.geneFound });
    		self._trigger("loadGenePage", null, params);
